...
function buildSliceKeyframes(
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    // Add text-shadow to make white text glitch visible with stronger RGB shift
    const shadowX = (Math.random() * 20 - 10).toFixed(1);
    const shadowY = (Math.random() * 10 - 5).toFixed(1);
    frames += `${pct}% { 
      clip-path: inset(${Math.max(0, Math.min(100 - height, 50 + offset))}% 0 ${Math.max(0, height)}% 0); 
      filter: hue-rotate(${hue}deg);
      text-shadow: ${shadowX}px ${shadowY}px 0 rgba(255,0,0,1), -${shadowX}px -${shadowY}px 0 rgba(0,255,255,1);
    }\n`;
  }
  return frames;
}
...
export default function GlitchEffect({
  ...
  useEffect(() => {
    ...
    setKeyframes(buildSliceKeyframes(slice.minHeight, slice.maxHeight, true));
  ...
  return (
    <div ref={wrapperRef} id="glitch-effect-wrapper" style={mainStyle}>
      ...
      {/* Base text that is always fully visible */}
      <div style={{ ...mainStyle, width: "fit-content", position: "relative" }}>
        {content}
        
        {/* Sliced/Glitching overlay text */}
        <div
